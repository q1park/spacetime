...
class Simulator:
    @staticmethod
    def random(n_nodes, degree, graph_type, w_range, force_positive, seed):
    ...
    @staticmethod
    def sem(adj, n_samples, linear_type, sem_type, noise_scale, x_dims, seed):
        ...
        for j in list(nx.topological_sort(G)):
            ...
            if linear_type == 'linear':
                ...
                eta = np.cos(X[:, parents, 0] + 1).dot(adj[parents, j])
            elif linear_type == 'nonlinear_2':
                eta = (X[:, parents, 0]+0.5).dot(adj[parents, j])
            else:
                raise ValueError('unknown linear data type')

            if sem_type == 'linear-gauss':
                if linear_type == 'linear':
                    X[:, j, 0] = eta + np.random.normal(scale=noise_scale, size=n_samples)
                elif linear_type == 'nonlinear_1':
                    X[:, j, 0] = eta + np.random.normal(scale=noise_scale, size=n_samples)
                elif linear_type == 'nonlinear_2':
                    X[:, j, 0] = 2.*np.sin(eta) + eta + np.random.normal(scale=noise_scale, size=n_samples)
            elif sem_type == 'linear-exp':
                X[:, j, 0] = eta + np.random.exponential(scale=noise_scale, size=n_samples)
            elif sem_type == 'linear-gumbel':
                X[:, j, 0] = eta + np.random.gumbel(scale=noise_scale, size=n_samples)
            else:
        ...
        if x_dims > 1 :
            for i in range(x_dims-1):
                ...
                X[:, :, i+1] += np.random.normal(scale=noise_scale, size=(n_samples, adj.shape[0]))
            X[:, :, 0] = np.random.normal(scale=noise_scale, size=1) * X[:, :, 0] 
            X[:, :, 0] += np.random.normal(scale=noise_scale, size=1) 
            X[:, :, 0] += np.random.normal(scale=noise_scale, size=(n_samples, adj.shape[0]))
